...
#
# prerequisite in path: novoalign, samtools, Athlates (need lib64 of gcc>=5.4.0 in LD_LIBRARY_PATH), samtools (>=1.4), sambamba
...
my ($build,$reference,$fastq1,$fastq2,$fastq3,$fastq4,$star,$output,$thread,
  $mhc_i,$mhc_ii,$percentile_cutoff)=@ARGV;
...
my (%fastqs,$stringtie_parameters);
...
foreach $type (keys %fastqs)
{
  ...
  if ($type eq "tumor") {$stringtie_parameters="-f 0.15 -j 5 -c 7 -M 0.75";} else {$stringtie_parameters="-f 0.05 -j 2";}
  ...
  while ($line=<FILE>)
  {
    ...
    if ($line=~/(.*)\tStringTie\ttranscript.*\t([\+\-])\t.*gene_id\s\"(.*?)\";.*transcript_id\s"(.*)";\scov.*FPKM\s"(.*)";\sTPM/)
    {
      ...
      {
        if ($fpkm>1 || $type eq "normal") {print COORD $to_print;} # more lenient on normal samples
        $chr=$1;
        $strand=$2;
        $gene_id=$3;
        $transcript_id=$4;
  ...
  if ($fpkm>1 || $type eq "normal") {print COORD $to_print;}
  close(FILE);
  close(COORD);
  ...
  system_call($path."/script/TransDecoder/TransDecoder.LongOrfs -t ".$output."/".$type."/full_dna.txt -m 50 -S --output_dir ".
  ...
  system_call($path."/script/TransDecoder//TransDecoder.Predict -t ".$output."/".$type."/full_dna.txt --single_best_only ".
    "--retain_blastp_hits ".$output."/".$type."/cds_search/blastp.outfmt6 --output_dir ".$output."/".$type."/cds_search");

  %final_protein=(); # find good ORFs
...
{
  $line=~s/\n//;
  if ($line eq "") {next;} # compensate for a previous caveat in programming
  @items=split(" ",$line);
...
foreach $class (("I","II"))
{
  foreach $len (@{$neoantigen_len{$class}})
  {
    ...
    foreach $transcript_id (keys %final_protein)
    {
      ...
      while ($i<=length($final_protein{$transcript_id})-$len)
      {
        $j=look_ahead($i,$final_protein{$transcript_id},\$wt_proteins,\$normal_proteins,$len); # can safely jump this far
        if ($j<0) # don't jump. this position will lead to a new epitope 
        {
          ...
          if ($i<=5) {last;} 
          $epitope=substr $final_protein{$transcript_id},$i,$len;
          print FILE ">".$transcript_id."-".$i."-".($i+$len-1)."-NA-NA-NA-NA\n".$epitope."\n";
          $i++;
        }else
        {
          $i+=$j+1;
...
system_call("Rscript ".$path."/script/splicing_cleanup.R ".$output."/binders/neoantigen ".$output." ".$path."/script");
...
if (1==1)
{
  ...
  system_call("cp ".$output."/tumor/out.gtf ".$output);
